...
def index():
    ...
    # 将message中的内容渲染为HTML
    markdown_message = markdown.markdown(formatted_message)
...
def get_information():
    ...
    message[history_id_input] = {
        "history_content": [{"role": "system", "content": prompt_input}],
...
def add_output_streamly(content, history_id):  # 添加回复
    ...
    if message[history_id]["history_content"][-1]["role"] == "user":
        ...
        # 如果有，就替换为最新的
        message[history_id]["history_content"][-1] = {
            "role": "assistant",
            "content": content,
        }
...
def create_image(
    prompt,
):  # 鉴于接口返回url（oaidalleapiprodscus.blob.core.windows.net）暂时没有被墙，所以不必下载下来再展示，直接在网页上展示即可
    ...
    if message[history_id]["history_content"][-1]["role"] == "user":
        ...
        for chunk in completion:
            ...
            if chunk.choices[0].delta.content is not None:
                full_text += chunk.choices[
                    0
                ].delta.content  # 添加这一部分回复到完整回复
